...
export default function GameLoop(entities, {touches, time, dispatch}) {
  ...
  for(let i = 1; i < 3; i++) {
    ...
    if(entities.box.body.position.y <= entities[`wallLeft${i}`].body.bounds.max.y && 
    ...
    if(entities[`wallLeft${i}`].body.bounds.max.y >= vars.MAX_HEIGHT + 60) {
      const pipeA = getWalls(-vars.MAX_HEIGHT)
      entities[`wallLeft${i}`].point = false
      Matter.Body.setPosition(entities[`wallLeft${i}`].body, pipeA.pipeLeft.pos)
      Matter.Body.setPosition(entities[`wallRight${i}`].body, pipeA.pipeRight.pos)
    }

    Matter.Body.translate(entities[`wallLeft${i}`].body, {x: 0, y: vars.WALL_SPEED})
    Matter.Body.translate(entities[`wallRight${i}`].body, {x: 0, y: vars.WALL_SPEED})
  }

  // check for game over
  const boxPosX = entities.box.body.position.x
  const boxPosY = entities.box.body.position.y
  if(boxPosX < 0 || boxPosX > vars.MAX_WIDTH || boxPosY < 0 || boxPosY > vars.MAX_HEIGHT) {
